...
- The pre-existing `TheSqlODataMcp.IntegrationTests` `NU1903` restore failure (unrelated `SSH.NET` advisory) was resolved separately; see ADR 0004's subsequent-evidence note.

## Subsequent evidence

While reviewing a later, unrelated slice (ADR 0011), the primary agent's new test classes increased Core.Tests' xUnit parallelism enough to intermittently expose a real thread-safety defect in this slice's own code: `SemanticOverlayImporter`'s shared static `JsonSchema` instance was not safe to `Evaluate` concurrently for this `$ref`-heavy schema. A dedicated stress probe (2,000 concurrent imports of a document with exactly one schema-only violation) measured roughly 42% silently missing the expected `SchemaViolation` error — a genuine validation bypass under concurrent load (e.g. an ASP.NET Core host handling simultaneous catalog-overlay uploads), not merely flaky test output. If the schema violation is the *only* problem in an otherwise-typed-deserializable overlay (a missing required field, an invalid enum value), this race could let an invalid overlay be wrongly accepted.

Fixed on 2026-08-17 by serializing the `Schema.Evaluate` call with a `lock` (schema evaluation is not a query-path hot loop; catalog-overlay import is an administrative, occasional operation, so serializing it has no meaningful throughput cost). The stress probe was re-run against the fix: 0/2,000 concurrent imports missed the violation. A permanent 200-iteration regression test (`ConcurrentImportsReliablyDetectTheSameSchemaViolation` in `SemanticOverlayImporterQaTests.cs`) guards against recurrence — at the original ~42% failure rate, 200 clean iterations passing by chance is not a realistic risk. This fix is bundled with the ADR 0011 commit; see that ADR's acceptance evidence for the corresponding CI run.
