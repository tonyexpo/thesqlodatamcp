...
**Status:** In progress. The provider-neutral technical catalog model, deterministic structural hash, SQL Server catalog type mapper, and complete table/view/column/key/index/foreign-key introspection are accepted, with real SQL Server CI evidence (GitHub Actions run [30031601860](https://github.com/tonyexpo/thesqlodatamcp/actions/runs/30031601860)). Semantic overlay Markdown/YAML import and validation (ADR 0010) is implemented and locally validated, pending CI. The remaining catalog lifecycle — overlay merge into the technical catalog, capability/revision models, SQLite persistence, activation/rollback, and search — is pending.
