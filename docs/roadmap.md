...
**Status:** In progress. The provider-neutral technical catalog model, deterministic structural hash, SQL Server catalog type mapper, and first table/view/column introspection slice are accepted. Relational metadata is implemented and locally validated but still awaits real SQL Server CI acceptance; the remaining catalog lifecycle is pending.
