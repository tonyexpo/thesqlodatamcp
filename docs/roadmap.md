...
**Status:** In progress. The provider-neutral technical catalog model, deterministic structural hash, SQL Server catalog type mapper, complete table/view/column/key/index/foreign-key introspection, semantic overlay Markdown/YAML import and validation (ADR 0010), the overlay merge into the technical catalog (ADR 0011), and the catalog revision lifecycle model (ADR 0012) are all accepted with real CI evidence (GitHub Actions runs [30031601860](https://github.com/tonyexpo/thesqlodatamcp/actions/runs/30031601860), [32059087651](https://github.com/tonyexpo/thesqlodatamcp/actions/runs/32059087651), and [32064882285](https://github.com/tonyexpo/thesqlodatamcp/actions/runs/32064882285); ADR 0012's run is pending). The remaining catalog lifecycle — SQLite persistence, atomic activation/rollback, bootstrap modes, and search — is pending.
